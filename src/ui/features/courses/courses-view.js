        ...
        ${courses.map((course) => {
          ...
          return `<article class="ui-course" data-active="${active}">
            ...
            <div class="ui-course__controls">
              ${active ? `<button class="ui-btn" data-command="stop-course" data-id="${escapeHtml(course.id)}">Stop</button>` : `<button class="ui-btn ${canPurchase ? '' : 'ui-btn--primary'}" data-command="run-course" data-id="${escapeHtml(course.id)}">Run</button>`}
              ${canPurchase ? `<button class="ui-btn ui-btn--primary" data-command="purchase-course" data-id="${escapeHtml(course.id)}">Buy +1</button>` : ''}
