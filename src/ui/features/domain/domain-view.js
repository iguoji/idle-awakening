...
const PAGE_CONFIG = {
  shop: {
    title: 'Shop', kicker: 'Economy', description: 'Purchase permanent upgrades and resources from the game economy.',
    queries: [['query-items-data', {}]], responses: ['items-data'],
    primary: { command: 'purchase-item', label: 'Purchase' },
  },
  inventory: {
    title: 'Inventory', kicker: 'Storage', description: 'Inspect carried items and perform item actions without exposing engine internals to the UI.',
    queries: [['query-inventory-data', {}]], responses: ['inventory-data'],
    actions: [{ command: 'consume-inventory', label: 'Use', amount: 1 }, { command: 'sell-inventory', label: 'Sell', amount: 1 }],
  },
  property: {
    title: 'Property', kicker: 'Home', description: 'Manage furniture, permanent effects and property progression.',
  ...
  workshop: {
    title: 'Workshop', kicker: 'Production', description: 'Run crafting and alchemy recipes through the extracted worker runtime.',
  ...
  spellbook: {
    title: 'Spellbook', kicker: 'Magic', description: 'Cast unlocked spells and inspect magical progression.',
    queries: [['query-spell-data', {}]], responses: ['spell-data'],
    primary: { command: 'use-spell', label: 'Cast' },
  },
  social: {
    title: 'Social', kicker: 'Guilds', description: 'Choose a guild and purchase unlocked guild upgrades.',
    queries: [['query-guild-items-data', {}]], responses: ['guild-items-data'],
  },
  world: {
    title: 'World', kicker: 'Exploration', description: 'Explore the generated map and inspect the current exploration state.',
    queries: [['query-map-data', {}], ['map-query-general-data', {}]], responses: ['map-data', 'map-general-data'],
...
function renderItemCard(item, config) {
  ...
  if (config.primary?.command === 'purchase-furniture') controls = renderButton(config.primary.command, config.primary.label, item, { filterId: 'furniture' });
...
function renderListBlock(label, list, config, limit = 100) {
  return `<article class="ui-card"><div class="ui-card__body"><div class="ui-section-title"><div><strong>${escapeHtml(label)}</strong><span>${list.length} entries</span></div></div><div class="ui-domain-list">${list.slice(0, limit).map((item) => renderItemCard(item, config)).join('')}</div>${list.length > limit ? '<p class="ui-muted">Showing the first 100 entries.</p>' : ''}</div></article>`;
}
...
function renderSocial(data) {
  ...
  const upgrades = Array.isArray(data.availableUpgrades) ? data.availableUpgrades.flat?.(2) || data.availableUpgrades : [];
...
export function renderDomainView(view, snapshot) {
  ...
  return `<section class="ui-page-head"><div><div class="ui-kicker">${escapeHtml(config.kicker)}</div><h1>${escapeHtml(config.title)}</h1><p>${escapeHtml(config.description)}</p></div><div class="ui-status-pill">${escapeHtml(readiness)}</div></section><section class="ui-grid ui-grid--single">${blocks.filter(Boolean).join('')}</section>`;
}
